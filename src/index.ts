...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'F1');
}

// Ergast (ergast.com/api/f1) was discontinued end-2024. Jolpica runs a
// drop-in clone at api.jolpi.ca with the same JSON shape, so this is a
// one-line fix — no response remapping needed.
...
async function getCurrentStandings() {
  const res = await pwFetch(`${BASE_URL}/current/driverStandings.json`);
  if (!res.ok) throw await httpError(res, 'Ergast API error');
...
async function getRaceResults(season: string, round: string) {
  const res = await pwFetch(`${BASE_URL}/${encodeURIComponent(season)}/${encodeURIComponent(round)}/results.json`);
  if (!res.ok) throw await httpError(res, 'Ergast API error');
...
async function getSchedule(season: string) {
  const res = await pwFetch(`${BASE_URL}/${encodeURIComponent(season)}.json`);
  if (!res.ok) throw await httpError(res, 'Ergast API error');
...
async function getDriver(driverId: string) {
  const res = await pwFetch(`${BASE_URL}/drivers/${encodeURIComponent(driverId)}.json`);
  if (!res.ok) throw await httpError(res, 'Ergast API error');
